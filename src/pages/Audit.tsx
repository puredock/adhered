...
const Audit = () => {
  ...
  const filteredFrameworks = regulatoryFrameworks.filter(framework => {
    const matchesSearch = framework.name.toLowerCase().includes(searchQuery.toLowerCase()) || framework.description.toLowerCase().includes(searchQuery.toLowerCase());
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-80">
                ...
                <Input type="search" placeholder="Search frameworks..." className="pl-9" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
              </div>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  {allStatuses.map(status => <DropdownMenuCheckboxItem key={status} checked={selectedStatuses.includes(status)} onCheckedChange={() => toggleStatus(status)}>
                      {status === "active" ? "Active" : "Pending Review"}
                    </DropdownMenuCheckboxItem>)}
                  <DropdownMenuSeparator />
                  <DropdownMenuLabel>Filter by Tags</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {allTags.map(tag => <DropdownMenuCheckboxItem key={tag} checked={selectedTags.includes(tag)} onCheckedChange={() => toggleTag(tag)}>
                      {tag}
                    </DropdownMenuCheckboxItem>)}
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        <Card className="shadow-card border-border">
          <CardHeader>
            ...
            <CardDescription>
              ...
              {(selectedTags.length > 0 || selectedStatuses.length > 0) && <Button variant="link" size="sm" onClick={() => {
              setSelectedTags([]);
              setSelectedStatuses([]);
          ...
          <CardContent>
            {viewMode === "grid" ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredFrameworks.map(framework => <Link key={framework.id} to={`/audits/${framework.id}`}>
                    <div className="p-5 rounded-lg bg-card border border-border hover:border-primary hover:bg-accent/30 transition-all cursor-pointer group h-full">
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        {framework.tags.map(tag => <Badge key={tag} variant="secondary" className="text-xs">
                            {tag}
                          </Badge>)}
                      </div>
