...
import { Network, Shield, AlertTriangle, Activity, Home, Search, Loader2 } from "lucide-react";
                  ...
                  <Link key={network.id} to={`/networks/${network.id}`}>
                    <div className="flex items-center justify-between p-4 rounded-lg bg-card border border-border hover:border-primary hover:bg-accent/30 transition-all cursor-pointer group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="font-semibold group-hover:text-primary transition-colors">{network.name}</h3>
                          <p className="text-sm text-muted-foreground">
                            {network.device_count} devices • Last scan {formatTimeAgo(network.last_scan)}
                          </p>
                        </div>
                      </div>
                      <div>
            ...
            {devicesLoading ? (
              ...
              <div className="space-y-4">
                {devices.map((device, index) => (
                  <Link key={device.id} to={`/networks/${device.network_id}/devices/${device.id}`}>
                    <div className="flex items-center justify-between p-4 rounded-lg bg-card border border-border hover:border-primary hover:bg-accent/30 transition-all cursor-pointer group">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${getIconColor(index)} group-hover:scale-110 transition-transform`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {device.manufacturer} {device.model && `• ${device.model}`} • {device.ip_address}
                          </p>
                        </div>
                      </div>
