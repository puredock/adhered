...
import { AlertTriangle, CheckCircle2, ChevronDown, ChevronRight, ClipboardCheck, Clock, History, Loader2, ScanLine } from "lucide-react";
...
export function ActivityViewer({ deviceId, scans = [], audits = [] }: ActivityViewerProps) {
  ...
  const getActivityTypeIcon = () => {
    return activityType === "scans" ? <ScanLine className="w-4 h-4" /> : <ClipboardCheck className="w-4 h-4" />;
  };
  ...
  const renderActivityEntry = (activity: ActivityEntry) => {
    ...
    return (
      <div key={activity.id} className="space-y-2">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                {activity.vulnerabilitiesFound !== undefined && activity.vulnerabilitiesFound > 0 && (
                  <>
                    <span>•</span>
        ...
        {isSelected && activity.status === "running" && (
          <div className="mt-3">
            <PenetrationTestLog
              scanId={activity.id}
              onComplete={(status) => {
        ...
        {isSelected && activity.status !== "running" && (
          <div className="mt-3 p-6 rounded-lg border border-muted bg-muted/30">
            <p className="text-sm text-muted-foreground text-center">
              Logs are only available for ongoing activities.
              Completed logs will be available in the full scan report.
            </p>
          </div>
        )}
      </div>
