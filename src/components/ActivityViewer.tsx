...
export function ActivityViewer({
	...
	const [scanStates, setScanStates] = useState<
		Record<string, { steps: any[]; logs: any[] }>
	>({});
	...
	const ongoingActivities = currentActivities.filter(
		(a) => a.status === "running",
	);
	const recentActivities = currentActivities.filter(
		(a) => a.status !== "running",
	);
				...
				<Tabs defaultValue="ongoing" className="w-full">
					<TabsList className="grid w-full grid-cols-2">
						<TabsTrigger value="ongoing">
							Ongoing{" "}
							{ongoingActivities.length > 0 && `(${ongoingActivities.length})`}
						</TabsTrigger>
						<TabsTrigger value="recent">
							Recent{" "}
							{recentActivities.length > 0 && `(${recentActivities.length})`}
						</TabsTrigger>
					</TabsList>
