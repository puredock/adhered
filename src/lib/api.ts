...
export interface Device {
  ...
  device_type: "medical_device" | "iot_device" | "network_device" | "workstation" | "server" | "unknown";
...
export interface ScanResult {
  id: string;
  device_id: string;
  scan_type: "vulnerability_scan" | "penetration_test" | "compliance_audit" | "owasp_top10" | "mitre_attack";
...
export const api = {
  ...
  devices: {
    list: (params?: { network_id?: string; device_type?: string; skip?: number; limit?: number }) => {
      const queryParams = new URLSearchParams();
  ...
  scans: {
    list: (params?: { device_id?: string; scan_type?: string; status?: string; skip?: number; limit?: number }) => {
      const queryParams = new URLSearchParams();
